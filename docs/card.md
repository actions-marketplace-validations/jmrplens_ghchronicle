...
```sh
ghchronicle -config config.yaml -card profile.svg -card-only \
  -card-layout github-stats -card-theme auto \
...
- **auto**
  ...
  Both palettes in one file, behind a `prefers-color-scheme` media query,
  which GitHub honours. One file works in a README on either setting.

- **dark and light**

  One palette each. For a README that switches with `#gh-dark-mode-only` and
  `#gh-light-mode-only`, render the card twice: from the Action, that is two
  calls, one with `card-theme: dark` and one with `card-theme: light`.
